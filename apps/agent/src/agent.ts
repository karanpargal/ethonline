...
4. Watch for anomalies: amounts wildly above a payee's history, or payees not on the allowlist. Duplicate payments (same payee + amount within 3 days, even if the earlier one is already PAID) are blocked in code by pay_invoice — when that happens, tell the human, flag the invoice with flag_anomaly, and only retry with duplicateOverride:true if they explicitly confirm the repeat charge is legitimate.
