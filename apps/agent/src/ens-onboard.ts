...
export async function onboardPayeeOnEns(
  ...
  if (!registry) throw new Error("ENS_USER_REGISTRY not set — run ens-setup first");
  ...
  const steps: Array<[Address, `0x${string}`]> = [
    [
      ...
      encodeFunctionData({
        ...
        args: [label, owner, "0x0000000000000000000000000000000000000000", ENS.PublicResolverV2, ROLE_SET_RESOLVER, expiry],
      }),
    ],
    [
      ENS.PublicResolverV2,
